...
- **Session** — `/goon edge` builds a meter and streak. Finish cashes it. Ruin dumps it (yours or theirs). Chat/VC/jobs/`daily` also fill the meter. Every ~2 hours the main chat gets a group-session call (house pot); first answer wins goonbux + condoms, then a join round. Condoms block ruins, hold leaks, and keep streak on finish.
