...
- **Session** — `/goon edge` builds a meter and streak. Finish cashes it. Ruin dumps yours or theirs. Chat/VC/jobs/`daily` also fill the meter. Every 145 minutes the main chat gets a group-session call with Velvet art (house pot); first answer gets kisses or head from Velvet (follow-up image), then a join round. **First two joiners each get a GoonCard.** Trivia winners also get a card. Condoms block ruins, hold leaks, and keep streak on finish.
